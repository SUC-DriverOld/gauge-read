...
MAX_LOG = 100
LOG_DIR = ".logs"
LOG_FILENAME_ENV = "GAUGE_LOG_FILE"
...
def manage_log_files(log_dir, max_log):
    def parse_date(filename):
        ...
        for fmt in ("%Y-%m-%d", "%Y-%m-%d_%H-%M-%S"):
            try:
                return datetime.strptime(basename.split(".")[0], fmt)
            except ValueError:
                continue
        return datetime.min
    ...
    active_log_filename = os.environ.get(LOG_FILENAME_ENV)
    log_files = sorted([f for f in os.listdir(log_dir) if f.endswith(".log")], key=parse_date)
    for log_file in log_files:
        if log_file == active_log_filename:
            continue
        compress_log_file(os.path.join(log_dir, log_file))

    archive_files = [f for f in os.listdir(log_dir) if f.endswith(".log") or f.endswith(".log.gz")]
    archive_files = sorted(archive_files, key=parse_date)

    while len(archive_files) > max_log:
        try:
...
def get_logger(console_level=logging.INFO, max_log=MAX_LOG):
    ...
    console_formatter = ColorFormatter(
        fmt="%(asctime)s.%(msecs)03d %(levelname)s %(location)-20s %(message)s", datefmt="%H:%M:%S"
    )
    ...
    file_formatter = logging.Formatter(
        fmt="[%(asctime)s.%(msecs)03d] [%(levelname)s] [%(pathname)s:%(lineno)d] %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
    )
