...
class BaseOptions(object):
    def __init__(self):
        ...
        self.parser.add_argument(
            "--num_workers", default=cfg.get("num_workers", 4), type=int, help="Number of workers used in dataloading"
        )
        self.parser.add_argument("--cuda", default=cfg.get("cuda", True), type=str2bool, help="Use cuda to train model")
        self.parser.add_argument(
            "--mgpu", action="store_true", help="Use multi-gpu to train model", default=cfg.get("mgpu", True)
        )
        ...
        self.parser.add_argument(
            "--pretrain", default=cfg.get("pretrain", True), type=str2bool, help="Pretrained AutoEncoder model"
        )
        self.parser.add_argument(
            "--viz", action="store_true", default=cfg.get("viz", False), help="Whether to output debug info"
        )

        # train opts
        self.parser.add_argument("--max_epoch", default=cfg.get("max_epoch", 100), type=int, help="Max epochs")
        self.parser.add_argument(
            "--lr", "--learning-rate", default=cfg.get("lr", 1e-4), type=float, help="initial learning rate"
        )
        self.parser.add_argument(
            "--lr_adjust",
            default=cfg.get("lr_adjust", "fix"),
            choices=["fix", "poly"],
            type=str,
            help="Learning Rate Adjust Strategy",
        )
        self.parser.add_argument(
            "--stepvalues", default=cfg.get("stepvalues", []), nargs="+", type=int, help="# of iter to change lr"
        )
        self.parser.add_argument(
            "--weight_decay", "--wd", default=cfg.get("weight_decay", 0.0), type=float, help="Weight decay for SGD"
        )
        ...
        self.parser.add_argument(
            "--optim", default=cfg.get("optim", "Adam"), type=str, choices=["SGD", "Adam"], help="Optimizer"
        )
        self.parser.add_argument("--save_freq", default=cfg.get("save_freq", 5), type=int, help="save weights every # epoch")
        self.parser.add_argument(
            "--display_freq", default=cfg.get("display_freq", 10), type=int, help="display training metrics every # iter"
        )
        self.parser.add_argument(
            "--viz_freq", default=cfg.get("viz_freq", 50), type=int, help="visualize training process every # iter"
        )
        self.parser.add_argument(
            "--log_freq", default=cfg.get("log_freq", 100), type=int, help="log to tensorboard every # iterations"
        )
        self.parser.add_argument(
            "--val_freq", default=cfg.get("val_freq", 100), type=int, help="do validation every # iterations"
        )

        # data set
        self.parser.add_argument(
            "--net",
            default=cfg.get("net", "vgg"),
            type=str,
        ...
        self.parser.add_argument(
            "--test_size", default=tuple(cfg.get("test_size", (512, 1024))), type=tuple, help="model input size"
        )

        # eval args
        self.parser.add_argument(
            "--checkepoch", default=cfg.get("checkepoch", 100), type=int, help="Load checkpoint number"
        )  # 110, 220
